...
import { Star, Bookmark, Check, Trash2, Share2, Filter } from "lucide-react";
...
const initialWatchlist: WatchlistItem[] = movies.map((m, i) => ({
  movie: m,
        ...
        {filtered.map(({ movie, status, progress }) => (
          ...
          >
            <Link to={`/movie/${movie.id}`}>
              <div
                className="w-16 h-24 rounded-lg flex-shrink-0 flex items-end p-1.5"
                style={{ background: movie.poster }}
              >
                <span className="text-[7px] font-bold text-foreground/80 leading-tight">{movie.title}</span>
              </div>
            </Link>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className="text-xs text-muted-foreground">{movie.runtime}</span>
                    <div className="flex items-center gap-0.5">
                      <Star className="h-3 w-3 text-star fill-star" />
                      <span className="text-xs text-star">{movie.rating}</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex gap-1 mt-2 flex-wrap">
                ...
                {movie.newRelease && (
                  <Badge className="text-[10px] bg-cinema-red/20 text-cinema-red border-none px-1.5 py-0">
                    New Release
                  </Badge>
                )}
              </div>
